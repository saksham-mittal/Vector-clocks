...
The following code is an implementation of vector clocks
    ...
    for(int i=0; i<total_evnts; i++) {
        if(ie > 0 && se > 0) {
            ...
            if(r >= probability_intrn_evnt) {
                ...
                // Concatenating the buffer with clock vector
                string temp_convert = "";
                for(int i=0; i<n; i++)
                    temp_convert += (to_string(clck[i]) + " ");
            ...
            if(ie == 0) {
                ...
                // The msg sent is the vector clock concatenated with the id
                string temp_convert = "";
                for(int i=0; i<n; i++)
                    temp_convert += (to_string(clck[i]) + " ");
                temp_convert += ("| " + to_string(id + 1) + "| " + to_string(i + 1) + ";");
    ...
    while(lp_break) {
        for(int i=0; i<max_conn; i++) {
            ...
            if(strlen(buffer) > 0) {
                ...
                // Sample buffer : "1 10 32 5 | 45 | 3;..."
                int ind = 0;
                while(buffer[ind] != '\0') {
                    char str;
                    vector<int> received_clck;
                    int var = 0, thread_send, msg_send;
                    ...
                    // Now compare the received vector clock at all index
                    // to the clock of the process

                    for(int i=0; i<n; i++)
                        clck[i] = max(clck[i], received_clck[i]);
...
void manage(int id) {
    ...
    thread rec_th(receiver, id, tm_clck);
    thread sdr_th(sender, id, tm_clck);
